...
Dispatcher::Dispatcher(Channel& disp2gate, Channel& disp2motor,
		Channel& disp2lamp, Channel& disp2emrec, Channel& disp2sens,
		Channel& disp2hight, Channel& disp2Remote, Channel& disp2TimerControl,
		Channel& disp2cali, Channel& disp2serial, Channel& disp2main, Channel& toDisp,  EXECMODE mode) :
		_2gate(disp2gate), _2motor(disp2motor), _2lamp(disp2lamp), _2emrec(
				disp2emrec), _2sens(disp2sens), _2hight(disp2hight), _2remoteControl(
				disp2Remote), _2TimerControl(disp2TimerControl), _2cali(disp2cali), _2serial(disp2serial), _2main(disp2main),  _2me(toDisp), _currentmode(
				mode), _calibration(false)  {
...
void Dispatcher::operator ()() {
	if (_currentmode == EXECMODE::MAIN) {
		...
		while (true) {
			...
			LOG_WRITE("Dispatcher received message.");
			std::cout << "Dispatcher, Signal: " << Enums::signalToString(rcvMsg.signal) << std::endl;

			if(rcvMsg.signal == Signal::STARTCALIBRATION) {
				_2cali.enqueue(rcvMsg);
				_calibration = true;
				std::cout << "dispatcher in calibration mode" << std::endl;
			}
			else if(rcvMsg.signal == Signal::STOPCALIBRATION) {
				_2cali.enqueue(rcvMsg);
				_calibration = false;
				std::cout << "Calibration in dispatcher deactivated" << std::endl; //TODO remove
			}
			else if(rcvMsg.signal == Signal::CALIBRATIONVALUES) {
				_calibration = false;
			}
			...
			switch (rcvMsg.dest) {
			case Component::MAIN:
				if(_calibration && (rcvMsg.src == Component::SENSOR || rcvMsg.src == Component::OTHER_MACHINE)) {
					std::cout << "To Calibration, Signal: " << Enums::signalToString(rcvMsg.signal) << std::endl;
					_2cali.enqueue(rcvMsg);
				}
			...
			case Component::HIGHT:
				...
				else if(rcvMsg.signal != Signal::LB_HIGHT_ENTERED && rcvMsg.signal != Signal::LB_HIGHT_LEFT){
					_2cali.enqueue(rcvMsg);
				}
				break;
			...
			case Component::CALIBRATION:
				LOG_WRITE("Message ist für Calibration. Dispatcher leitet weiter...");
				_2cali.enqueue(rcvMsg);
				break;
			case Component::OTHER_MACHINE:
				LOG_WRITE("Message ist für OtherMachine. Dispatcher leitet weiter...");
				_2serial.enqueue(rcvMsg);
				break;
			case Component::TIMER_CONTROL:
				LOG_WRITE("Message ist für TimerControl. Dispatcher leitet weiter...");
				_2TimerControl.enqueue(rcvMsg);
				break;
			default:
		...
		for (int i = 0; i < listSize; i++) {
			...
			//std::cout << "ACTUAL " << actualMessage.timestamp - donedeed
			//		<< std::endl;
			std::this_thread::sleep_for(
					std::chrono::milliseconds(
							actualMessage.timestamp - donedeed));
...
//			if (read.compare("ENDOFRECORDFILE") == 0) {
		...
		//helpfunctioncall
		sig = Enums::convertStringToSignal(read);
		std::getline(_readstream, read);
		dest = Enums::convertStringToComponent(read);
		std::getline(_readstream, read);
		src = Enums::convertStringToComponent(read);
